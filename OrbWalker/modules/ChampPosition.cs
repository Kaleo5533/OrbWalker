...
namespace OrbWalker.modules
{
    class ChampPosition
    {
        ...
        public static Point GetEnemyPosition(double screenWidth)
        {
            ...
            {
                Point[] OrderedY = Searched.OrderBy(t => t.Y).ToArray();
                ...
                for (int i = 0; i < array3.Length; i++)
                {
                    ...
                    SharpDX.Vector2 current = new SharpDX.Vector2(point.X, point.Y);
                    if ((from t in list where (t.Item1 - current).Length() < 25f || Math.Abs(t.Item1.X - current.X) < 25f select t).Count() < 1)
                    {
                        list.Add(new Tuple<SharpDX.Vector2, double>(current, (double)(current - new SharpDX.Vector2(FOV.X, FOV.Y)).Length()));
                        if (list.Count > 2)
                        {
                            break;
                        }
